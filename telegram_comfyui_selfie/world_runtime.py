...
class WorldRuntimeMixin:
    ...
    def _character_card_snapshot_for_key(self, session_id: str, character_key: str) -> tuple[dict[str, Any], bool]:
        """读取指定角色的卡片快照；活动角色取 live state，非活动角色只取角色池。"""
        state = self._get_session_state(session_id)
        key = str(character_key or "").strip()
        active_key = self._context_character_key(session_id) if hasattr(self, "_context_character_key") else ""
        if key == active_key:
            has_active_identity_override = any(
                session_schema.get_character_value(state, state_key, "") not in (None, "")
                for card_key, state_key in character_card.CARD_STRING_FIELDS
                if card_key in CHARACTER_VERSION_FIELDS
            )
            if (
                not key
                and not session_schema.get_character_value(state, "persona_user_set", False)
                and not has_active_identity_override
                and hasattr(self, "_default_character_payload")
            ):
                raw = self._default_character_payload()
                return {field: copy.deepcopy(raw.get(field)) for field in character_card.CARD_KEYS}, True
            card = character_card.card_from_state(state)
            if key:
                card["character"] = key
            return copy.deepcopy(card), True
        if key:
            saved = session_schema.get_saved_characters(state)
            raw = saved.get(key)
            if isinstance(raw, dict):
                card = {field: copy.deepcopy(raw.get(field)) for field in character_card.CARD_KEYS}
                card["character"] = key
                return card, True
            return {}, False
        if hasattr(self, "_default_character_payload"):
            raw = self._default_character_payload()
            return {field: copy.deepcopy(raw.get(field)) for field in character_card.CARD_KEYS}, True
        return {}, False

    def _character_snapshot_version(self, session_id: str, character_key: str) -> str:
        """生成角色配置版本指纹，供跨 await 的后台任务做轻量 CAS 校验。"""
        key = str(character_key or "").strip()
    ...
    def _character_life_profile_snapshot(
        self,
        session_id: str,
        character_key: str,
        *,
        ...
        """从指定角色卡和冻结上下文构造生活档案，绝不借用其他活动角色。"""
        card = card if isinstance(card, dict) else self._character_card_snapshot_for_key(session_id, character_key)[0]
        context = context if isinstance(context, dict) else {}
        cached = context.get("life_profile") if isinstance(context.get("life_profile"), dict) else {}
        age = self._normalize_age_stage(card.get("age_stage"))
        anchor = self._normalize_day_anchor(card.get("day_anchor") or card.get("occupation"))
        age = age or self._normalize_age_stage(cached.get("age_stage"))
        anchor = anchor or self._normalize_day_anchor(cached.get("day_anchor"))
        return {"age_stage": age or "unknown", "day_anchor": anchor or "unknown"}
    ...
    def _set_character_place(
        ...
        *,
        ...
        return self._set_character_place_for_key(
            session_id,
            character_key,
            key,
            matched,
    ...
    def _set_character_place_for_key(
        self,
        session_id: str,
        character_key: str,
        key: str,
        ...
        *,
        source: str = "auto",
        name: str = "",
        expected_character_version: str = "",
    ) -> bool:
        """按角色键写入 live 或冻结地点；版本不符时拒绝陈旧后台结果。"""
        if key not in PLACE_TYPES:
            return False
        state = self._get_session_state(session_id)
        character_key = str(character_key or "").strip()
        if expected_character_version and self._character_snapshot_version(session_id, character_key) != expected_character_version:
            logger.info("skip stale character place update session=%s character=%s", session_id, character_key)
            return False
        active_key = self._context_character_key(session_id) if hasattr(self, "_context_character_key") else ""
        target_state = state
        if character_key != active_key:
            context_key = character_key or "__default__"
            target_state = session_schema.get_character_contexts(state).get(context_key)
            if not isinstance(target_state, dict):
                logger.info("skip character place update without frozen context session=%s character=%s", session_id, character_key)
                return False
        updated_at = time.time()
        ...
        history = session_schema.get_character_place_history(target_state)
        if not history or history[-1].get("key") != key:
            session_schema.append_character_place_history(target_state, {
                "key": key,
    ...
    async def _update_character_place_from_text(
        self,
        session_id: str,
        text: str,
        *,
        ...
        expected_character_version: str = "",
        life_profile: dict[str, str] | None = None,
    ) -> bool:
        ...
        target_character_key = (
            ...
            if character_key is None and hasattr(self, "_context_character_key")
            else str(character_key or "").strip()
        )
        target_version = expected_character_version or self._character_snapshot_version(session_id, target_character_key)
        if not self._should_run_location_extract(text):
            return False
        if not self._bool_config("world_location_llm_extract", True) or not self.has_llm_config("image", session_id):
            return False
        system = (
        ...
        profile = copy.deepcopy(life_profile) if isinstance(life_profile, dict) else None
        if profile is None:
            state = self._get_session_state(session_id)
            active_key = self._context_character_key(session_id) if hasattr(self, "_context_character_key") else ""
            if target_character_key == active_key:
                profile = copy.deepcopy(self._life_profile(session_id))
            else:
                context = session_schema.get_character_contexts(state).get(target_character_key or "__default__") or {}
                card = self._character_card_snapshot_for_key(session_id, target_character_key)[0]
                profile = self._character_life_profile_snapshot(
                    session_id,
                    target_character_key,
                    card=card,
                    context=context if isinstance(context, dict) else {},
                )
        if key in ANCHOR_ONLY_PLACES and key not in self._profile_allowed_anchor_places(profile):
        ...
        return self._set_character_place_for_key(
            ...
            source="llm",
            name=place_name,
            expected_character_version=target_version,
        )
