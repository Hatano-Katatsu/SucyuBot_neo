...
CLEARUP_DONE_MSG = (
    ...
    "以及全部角色档案、长期记忆、日记、检查点均已清空，并已重置对话上下文。\n"
    "下一句起将以默认人设回应。"
)
OC_FIELD_ALIASES = {
    "名字": "name",
...
class CommandHandlersMixin:
    ...
    async def cmd_memory(self, chat_id, session_id, arg):
        ...
        if action in ("清空", "clear"):
            if rest != "确认":
                await self.send_message(chat_id, "这会删除当前角色的全部长期记忆。确认请发送: /记忆 清空 确认")
                return
            ...
            if hasattr(self, "_character_checkpoint_root") and hasattr(self, "_safe_checkpoint_part"):
                ...
                if checkpoint_dir.exists():
                    try:
                        ...
                        logger.warning("clearup 无法清理检查点目录：%s", checkpoint_dir, exc_info=True)
