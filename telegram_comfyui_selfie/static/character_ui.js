...
function renderRuntimeClothingPanel(char, isActive) {
  ...
  return `<section class="runtime-clothing-section wardrobe-studio">
    <aside class="outfit-preview-card">
      ...
      <button id="outfit-preview-generate" class="primary" type="button" disabled>生成搭配预览</button>
    ...
    <div class="wardrobe-workbench">
      ...
      <details class="runtime-clothing-pane wardrobe-add"><summary>添加衣物 / 描述换装</summary>
        ...
        <div class="wardrobe-editor-actions"><button class="primary" type="button" data-wardrobe-action="apply">直接换上</button><button type="button" data-wardrobe-action="save-closet">存进衣橱</button></div></div>
      </details>
      <details class="runtime-clothing-pane"><summary>衣物状态与穿搭管理 ${clothing.nudity ? `<span class="badge danger">${escapeHtml(clothing.nudity)}</span>` : ""}</summary>
        ...
        <div class="button-row"><button type="button" data-wardrobe-action="clear-item-states">还原衣物状态</button><button class="danger" type="button" data-wardrobe-action="clear">清空当前穿搭</button></div>
      </details>
      <details class="runtime-clothing-pane"><summary>公开场合备用穿搭</summary>
        ${wardrobeRows(clothing.public_fallback_outfit || {})}
        ${clothing.public_fallback_in_current ? '<button type="button" data-wardrobe-action="stash-public-fallback">从当前穿搭移出备用衣物</button>' : '<p class="muted">仅在公开场景需要时使用。</p>'}
      </details>
    </div>
  </section>`;
}
...
async function loadHistorySummary() {
  ...
  try {
    ...
    if (state.selectedCharacter !== rawCharKey || encodeURIComponent(state.selectedSession) !== sid) return;
