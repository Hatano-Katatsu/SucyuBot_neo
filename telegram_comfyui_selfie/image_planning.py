...
async def plan_roleplay_image(
    ...
    *,
    ...
    if needs_caption:
        if is_push:
            stable_front += (
                ...
                "caption 是角色随手发给用户的一条消息，不是日记或散文：通常只有一句话，最多两句，一般不超过 60 字。"
                "写法是一个具体细节加一个态度或反应，例如「楼下的猫又蹲在我鞋上了，赶都赶不走」。"
                "禁止叙述动作过程（不要「先…再…然后…」），一条里最多一个细节，不要用天气或温度收尾凑结尾。"
    ...
    if is_followup:
        ...
        if concrete_guides:
            push_dynamic_parts.append(
                "本次具体话题引导（动态，仅本次推送生效；选一条最合适的作为 caption 的明确主题即可，不必全部覆盖，"
                "也不得退回只有大类方向的泛泛内容。"
                "引导只是话题锚点：caption 只取它的核心话题，不要沿用引导的句式、长度或内部追问）:\n"
