...
- 最近一次全量测试：`py -3 -m unittest tests.test_core`，1179 tests / 1178 OK。唯一失败 `ExternalProxyTestCase::test_external_http_proxy_socks` 为**预先存在**的环境问题（`aiohttp` SOCKS 连接器需运行中事件循环，`git stash` 验证改动前同样失败），与本次改动无关。（注：本机 Bash 里的 `python` 指向 Windows Store 占位 stub，会以 exit 49 静默失败；跑测试用 `py -3` 或 `C:\Users\17122\AppData\Local\Programs\Python\Python311\python.exe`。）
