...
- 角色发/瞳被默认覆盖根治（2026-06-24）。**症状**：刻晴 base 是 `purple hair/purple eyes`，生图却画成 `black hair/brown eyes`；菜单改发/瞳「作用到全局」、WebUI 改身体特征「改不掉」。**真根因**（不同于上面 OC 创建污染那条）：`appearance.inject_appearance` 的 `resolve()` 让 `custom_default_hair`/`custom_default_eyes` 以 `override=True` **抹掉角色 base 发瞳再替换**——任何角色都被会话级默认发瞳覆盖（`custom_default_*` 不在角色卡 schema、跨角色不变=「全局」），且 WebUI 改 `positive_prefix` 也被它 override。**修复**：①`resolve()` 分两路——**已设角色**：角色 base 自己的发瞳优先，`custom_default` 只在角色没写该槽时兜底（绝不覆盖）；**默认角色态**：保留 `custom_default` 覆盖 config base 的原行为（改默认角色发色仍有效，见 `test_custom_hair_override_wins_over_scene_and_negative`）。②`/外型 发色`/`瞳色` 对已设角色改写进角色自己的 `custom_positive_prefix`（新增 `commands._replace_appearance_slot` 替换 hair/eyes 槽）并 `_snapshot_character` 回写卡（per-character，不再写全局默认）；非角色态才写 `custom_default_*`。新增 3 测试（角色发瞳优先 / 角色缺槽时默认兜底 / 菜单改发色写进角色卡）。全量 1363 tests OK（预存 1 SOCKS 错）。
- 数据结构重构·阶段 2.5（context 盒切换，2026-06-24）。第三个切换的盒：把 `chat_history` / `sent_photos_history` / `recent_message_history` / `checkpoint_summary` / `last_dream_at` / `replying_to_selfie` / `rounds_since_image` / `short_context_start` 等 17 个注册字段 + 3 个未注册字段（`last_message_text` / `last_message_time` / `character_history_summary`）共 **20 个扁平 transient 字段**收进 `state["context"]`。`session_schema` 新增 `ensure_context_box`（**非破坏懒迁移**：与 clothing/place 不同，context 选择**保留扁平键不弹出**，盒与扁平共存）+ `_CONTEXT_DEFAULT` + `_LEGACY_CONTEXT_FLAT_KEYS` + 全能访问器（`get_chat_history/set_chat_history/get_sent_photos_history/set_sent_photos_history/get_rounds_since_image/increment_rounds_since_image` 等 ~40 个）。`STATE_SCHEMA` 删除 17 个旧字段、加 `context`（T, reset_preserved=False）；3 个未注册字段补入默认值。`service._get_session_state` 挂接 `ensure_context_box`。**访问器策略**：读取优先扁平键（盒可能陈旧）、写入双写盒+扁平；向后兼容所有直写 `state["chat_history"]` 的旧代码/测试。`chat_context.py` ~33 个访问点、`scheduler_runtime.py` ~10 个、`image_planning.py` ~5 个、`commands.py` ~8 个、`service.py` ~7 个、`webui.py` ~3 个全部迁移走访问器。`scheduler_runtime.py` 补回 `import session_schema`。`_conversation_context_payload` / `_clear_transient_state` / `_restore_character_context` 按盒粒度工作（context 盒作为一个整体冻结/解冻/复位），语义不变。新增 `test_context_box_migration_and_accessors` 覆盖迁移往返 + 访问器 + 双写 + 向后兼容（扁平优先）+ 幂等 + clear_transient 复位。全量 1370 tests OK（预存 1 SOCKS 错）。
- 数据结构重构·阶段 3（session 盒切换，2026-06-24）。第四个切换的盒：把 `last_interaction` / `last_morning_greet_date` / `daily_trigger_times` / `daily_trigger_date` / `daily_triggered_times` / `saved_characters` / `character_contexts` / `init_flow` / `ntr_stage_reached` / `ntr_reconcile_count` / `ntr_affection_reset` / `frozen` / `frozen_at` 共 **13 个扁平全局字段**收进 `state["session"]`。`session_schema` 新增 `ensure_session_box`（**非破坏懒迁移**，与 context 盒同构：保留扁平键不弹出、盒与扁平共存）+ `_SESSION_DEFAULT` + `_LEGACY_SESSION_FLAT_KEYS` + 全套访问器（`get_last_interaction/set_last_interaction/get_frozen/set_frozen/get_saved_characters/get_character_contexts/get_daily_trigger_times/set_daily_trigger_times` 等 ~25 个）。**保留 13 个 `Field(G)` 注册**（区别于 clothing/place 的弹出式迁移）——扁平键继续被分类器归为全局 scope、永不随角色冻结，新增 `"session": Field(G)` 盒名也自动归 G，零分类器改动。容器访问器（`get_saved_characters`/`get_character_contexts`/`get_init_flow`）返回 live 同引用对象，保证 `saved[name]=card` 这类原地变更在盒与扁平两处一致。`last_interaction` 缺失时种 `time.time()`（保留 factory 语义）。`service._get_session_state` 挂接 `ensure_session_box`。5 个模块 ~62 个访问点全部迁移走访问器：`scheduler_runtime.py` ~18 个、`webui.py` ~14 个、`commands.py` ~13 个、`service.py` ~12 个、`telegram_io.py` ~3 个、`chat_context.py` ~2 个。`telegram_io.py` 补 `import session_schema`。`_clear_transient_state` / `_conversation_context_payload` 不动——`"session"` 盒名和 13 个扁平键全部 scope=G，遍历时自动跳过。新增 `test_session_box_migration_and_accessors` 覆盖迁移往返 + 访问器 + 双写 + 向后兼容（扁平优先）+ 容器原地变更一致性 + `last_interaction` factory 语义 + 幂等 + session 盒在 clear_transient 中不被清。更新 `test_state_schema_single_source_derives_sets` 的 `SESSION_GLOBAL_STATE_KEYS` 断言补上 `"session"`。全量 1379 tests OK（预存 1 SOCKS 错）。
