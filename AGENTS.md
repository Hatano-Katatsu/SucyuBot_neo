...
- 最近一次全量测试：`python -m unittest tests.test_core -v`，104 tests OK。
...
3. 加强测试不变量。重点覆盖：基础外观不含质量词/画师词/人数词；OC 不注入姓名 tag；一次性外观不持久化；正负提示词不冲突；自拍不露手机；对镜自拍才允许镜子和手机共存；亲密场景下 `is_intimate / partner_in_frame / device_in_frame` 标志正确透传。
