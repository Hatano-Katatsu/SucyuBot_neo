...
# 运行服务
py -3 -m telegram_comfyui_selfie --config data/config.json
...
- 最近一次全量测试：`py -3 -m unittest tests.test_core -v`，139 tests OK。（注：本机 Bash 里的 `python` 指向 Windows Store 占位 stub，会以 exit 49 静默失败；跑测试用 `py -3` 或 `C:\Users\17122\AppData\Local\Programs\Python\Python311\python.exe`。）
