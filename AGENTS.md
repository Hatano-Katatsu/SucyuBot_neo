...
- `view=selfie` 不是前摄自拍，而是「别人在画面外帮角色拍的照片」（角色看向镜头）。正向提示词只写 `A candid photo of a ..., looking at viewer, taken by someone else just out of frame`，绝不出现 `front-camera selfie`/`phone camera`/`selfie` 等措辞——那正是手机 UI 框的来源。手机/UI 的抑制全部交给负向提示词。只有对镜自拍（`view=mirror`）才允许同时出现镜子和手机。
