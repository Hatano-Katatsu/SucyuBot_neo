...
8. **公开场景外出兜底穿搭持久化**：公开场景 guard 若移除了 `slip dress` / `nightgown` 等私密主体衣物后只剩开衫、外套或配饰，不再只临时补 `modest casual clothes`；现在会补 `plain white crew-neck t-shirt, dark blue jeans`，写回当前 `wardrobe` / `dynamic_appearance` 并保存到会话状态，同时把兜底单品加入 closet。这样外出时兜底会自动作为当前穿搭出现，回到私密场景后仍可通过正常换装/脱衣链路移除，不影响海边泳装、角色 base 暴露造型或明确 public play 例外。
9. **本轮公开穿搭验证**：新增 `test_build_prompt_public_fallback_replaces_slip_dress_but_keeps_cardigan`，并更新公开睡衣兜底测试，覆盖 `black silk slip dress + white cotton knit cardigan` 在 izakaya/cafe 场景中会替换为完整外出兜底、保留开衫并持久化保存，同时回归私密场景保留睡衣、海边泳装、角色 base 暴露造型和明确 public play。验证 `py -3 -m compileall -q telegram_comfyui_selfie tests`、`node --check telegram_comfyui_selfie\static\app.js`、`py -3 -m py_compile scripts\compare_llm_chat_prompts.py` 与 `py -3 -m unittest tests.test_core -v`，结果 `Ran 379 tests in 9.219s`，`OK (skipped=1)`。
