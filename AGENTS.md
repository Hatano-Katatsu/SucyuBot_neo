...
4. **无参数 `/配图` 场景锚点修正**：最近一次成功发图单独作为当前场景终点，更早图片降为仅避重摘要；无明确场景参数时禁止从浴后阶段退回正在洗澡，检测到明确倒退只重试一次；用户明确指定新场景时仍由本次要求覆盖锚点。
5. **本轮追加验证**：新增无参数场景锚定/倒退重试与明确参数覆盖测试；`py -3 -m unittest tests.test_core -q` 结果 `425 tests, OK (skipped=1)`，`py -3 -m py_compile telegram_comfyui_selfie\image_planning.py` 通过，`git diff --check` 通过。
