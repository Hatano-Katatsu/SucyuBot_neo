...
- 亲密场景配图规则：`image_planning.py` 新增中文亲密关键字检测和 `is_intimate` 标志；图片规划器在亲密场景下固定 POV 视角、人物优先（表情/身体反应/用户身体局部）、环境精简、近景特写。`is_intimate` 标志通过 `tool_generate_image → _do_generate → do_generate → build_prompt` 管道透传，`build_prompt` 内与英文关键字 OR 作为 fallback。亲密 scene 下正向上剥离 `solo`、注入 `partial male body visible, male hands, male torso` 等男性存在感标签；负向上移除 `male`（允许男性身体局部出现）、改为压制 `third-person perspective`。
...
- 最近一次全量测试：`python -m unittest tests.test_core -v`，97 tests OK。

## 下一阶段目标

1. 强化场景结构化。保留自然语言场景描述，同时在内部拆出 `location/action/pose/light/props/outfit_once/forbidden` 等子字段，减少外貌混入场景、手机镜子规则失效、道具冲突等问题。
2. 完善推送与动线衔接。推送前应读取最近上下文、当前世界状态、用户位置和角色动线，生成"自然接上"的场景，而不是只按时间模板随机换场。
3. 加强测试不变量。重点覆盖：基础外观不含质量词/画师词/人数词；OC 不注入姓名 tag；一次性外观不持久化；正负提示词不冲突；自拍不露手机；对镜自拍才允许镜子和手机共存；亲密场景下 `is_intimate` 标志正确透传。
