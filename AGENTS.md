...
**当前进度（数据结构分盒重构）**：阶段 1（scope 表单一来源）✅、`clothing` 盒切换 ✅、持久裸体态 ✅、穿搭串归一（去重+空格）✅、OC 创建发瞳过滤 ✅。下一步按「## 数据结构分盒重构·路线图」砌 `place`→`context`→`session`→`character` 四盒。
...
| `place` | 短期态 | 14 | ~39 | ⬜ 待切 | **world_runtime 为主**（最集中、最自洽，建议先切） |
...
1. **`place`（先做）**——字段多但消费几乎全在 `world_runtime.py`，自洽、好测；位置子模型（user/character 各 value/label/text/updated_at/confidence）顺势可收成结构化子对象（这也是「数据结构阶段 2」位置子对象）。
