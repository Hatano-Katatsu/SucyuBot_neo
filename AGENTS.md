...
- `_call_llm` / `_call_llm_messages` 接受 `timeout` 覆盖，语义是**只放宽不收紧**：取 profile 超时与传入值的较大者，个别调用不能把 profile 配置的长超时改短。
- 长文摘要类后台任务（角色历史提要、dream 日记）输入或输出可达上千字，而部分聊天模型强制思考（`thinking_locked`）会让 `disable_thinking=True` 失效，思考与正文共享预算，profile 默认额度/超时内跑不完，表现为 `completion_tokens=0` 的超时。这类任务统一走 `_call_summary_text_llm`：显式放宽 `max_tokens`/`timeout`，并按 chat→fast 回落（与 `_call_memory_json_llm`、`_call_life_plan_json` 同一策略）。任何单一 purpose 的长文后台任务都不应只依赖 chat 模型，否则一个 profile 的思考行为就能让该管线长期停更。思考长度不可预测，预算按总量给足，正文长度由提示词目标字数和各自的字符硬上限约束，不靠 `max_tokens` 截断——截断只会得到半截摘要。
...
- 规划器一次性换装（`new_appearance_tags` 过显式换装闸：本轮请求参数或最近对话含明确换装词）按槽位覆盖本图的衣柜标签（连衣裙与上衣/下装互斥，未撞槽保留），并在图片成功后经 `outfit_commit` 提交衣柜、解除裸体与撞槽部件状态；发/瞳/other 槽不随单图提交。
- 场景衣物清洗的状态谓语尾巴不得越过 while/when/as 等从句连接词，避免把衣物后的人物动作一起删除。
